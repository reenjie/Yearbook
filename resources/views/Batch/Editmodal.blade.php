    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h6 class="modal-title" id="exampleModalLabel">Edit Batch</h6>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <form action="{{route('editbatch')}}" method="post">
          @csrf
        <div class="modal-body">
          <input type="text" value="{{$item->Name}}" name="name" required class="form-control mb-2" placeholder="Section">
            <input type="hidden" name="id" value="{{$item->id}}">
          <textarea name="description" placeholder="Description .." class="form-control mb-2"  id=""  rows="3">{{$item->Description}}</textarea>
